...
2. **Хост читает кабель.** В журнале панели раз в пять секунд появляется строка вида
   `audio: … peak=A/B`. `A` — уровень, который хост отправляет на iPad, `B` — контрольный уровень
   того же кабеля, снятый окном панели независимо. Оба числа от 0 до 32767.

   | Показание | Что это значит |
   |---|---|
   | `peak=0/0` | в кабель ничего не поступает — вернитесь к пункту 1 |
   | `peak=0/большое` | захват хоста залип; он перезапустится сам через 8 секунд, в журнале появится `capturing silence` |
   | оба большие | звук уходит на iPad, причина дальше |

3. **Звук доходит до приложения на iPad.** `node tools/ssh.js "cat /tmp/ipaddisplay.log"`: там должны
   расти счётчики `audio chunk #N` и стоять `started=1`.
