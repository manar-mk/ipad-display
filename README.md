...
Системный звук идёт на iPad как PCM 22050 Гц моно. Чтобы отправить на iPad только нужное приложение,
назначьте ему вывод «CABLE Input» (Windows: Параметры → Система → Звук → Микшер громкости) или
«iPad Display + динамики» (macOS, кнопка в панели создаёт это устройство). Устройство существует
только пока идёт передача.
