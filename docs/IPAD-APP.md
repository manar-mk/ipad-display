...
**Русский** · [English](IPAD-APP.en.md)
...
происходит само: хост находит iPad и начинает передачу. Язык интерфейса приложение берёт из настроек
самого iPad (русский или английский).

Приложение продолжает работать в фоне (`UIBackgroundModes: audio` плюс беззвучный keep-alive поток),
поэтому выход на домашний экран и блокировка больше не рвут соединение.
...
| `launch failed (11): timeout` | SpringBoard не смог выдать приложению сцену — обычно заблокированный экран или подвисший экземпляр: `node tools/ssh.js "killall IPadDisplay"` и запустить заново |
| Чёрный экран, процесс жив, порт 7801 отказывает | Залип `mediaserverd`, и вызовы звука в него не возвращаются: `node tools/ssh.js "killall mediaserverd"`, затем запустить приложение снова |
...
Всё, что обращается к `mediaserverd` — `AVAudioSession` и любые вызовы `AudioQueue` — должно
выполняться вне главного потока. Это синхронный IPC, и залипший демон иначе намертво остановит
`viewDidLoad`, пока SpringBoard не убьёт приложение с «failed to scene-create».
