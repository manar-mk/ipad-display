...
(function (global) {
  const RU = {
    ...
    'help.s4': '<b>Звук:</b> назначьте приложению устройство вывода iPad — «CABLE Input» на Windows (Параметры → Система → Звук → Микшер громкости), «iPad Display» на macOS (Системные настройки → Звук → Вывод, либо в настройках самого приложения). Устройства появляются только пока идёт передача.',
    ...
    'conn.locked': 'iPad заблокирован: разбудите его и разблокируйте — дальше приложение откроется само.',
    ...
    'a.makeDevice': 'Сделать iPad звуковым устройством',
    'a.makeDeviceHint': 'Создаёт устройства вывода «iPad Display» (звук только на iPad, как у монитора с колонками) и «iPad Display + динамики» (на iPad и на Mac сразу) и переключает вывод Mac на первое. Нужен виртуальный кабель: brew install blackhole-2ch.',
    'a.touch': 'касания с iPad как мышь',
    'a.manageDisplay': 'монитор только на время работы',
    'a.manageAudio': 'убирать звуковые устройства iPad при остановке',
  ...
  const EN = {
    ...
    'help.s4': '<b>Sound:</b> point an app’s output at the iPad device — “CABLE Input” on Windows (Settings → System → Sound → Volume mixer), “iPad Display” on macOS (System Settings → Sound → Output, or in the app’s own settings). These devices only exist while streaming.',
    ...
    'conn.locked': 'The iPad is locked: wake it and unlock — the app then opens by itself.',
    ...
    'a.makeDevice': 'Make the iPad a sound device',
    'a.makeDeviceHint': 'Creates the output devices “iPad Display” (sound only on the iPad, the way a monitor with speakers behaves) and “iPad Display + speakers” (both at once), and switches the Mac output to the first one. Needs the virtual cable: brew install blackhole-2ch.',
    'a.touch': 'iPad touches drive the mouse',
    'a.manageDisplay': 'monitor only while streaming',
    'a.manageAudio': 'remove the iPad audio devices when stopped',
